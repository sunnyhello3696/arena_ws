...
void PedsimMovement::OnInitialize(const YAML::Node &config){
    ...
    safety_dist_config = YAML::LoadFile(ros::package::getPath("arena_simulation_setup") + "/configs/saftey_distance_parameter_none.yaml");
...
void PedsimMovement::agentCallback(const pedsim_msgs::AgentStatesConstPtr& agents){
    // std::cout << "CALLBAKC RECEIVED" << std::endl;
