...
class TM_Random(TM_Obstacles):
    ...
    def reset(self, **kwargs) -> Obstacles:
        ...
        models_list = kwargs.get("MODELS_STATIC_OBSTACLES", self._config.MODELS_STATIC_OBSTACLES) \
            or self._PROPS.model_loader.models
        model_count = len(models_list)
        average_weight = 1.0 / model_count if model_count > 0 else 0
        # 为每个模型创建一个字典，其中键是模型名称，值是权重
        MODELS_STATIC_OBSTACLES: Dict[str, float] = dict.fromkeys(
            models_list,
            average_weight,
        )

        models_list = kwargs.get("MODELS_INTERACTIVE_OBSTACLES", self._config.MODELS_INTERACTIVE_OBSTACLES) \
            or self._PROPS.model_loader.models
        model_count = len(models_list)
        average_weight = 1.0 / model_count if model_count > 0 else 0
        ...
        models_list = kwargs.get("MODELS_DYNAMIC_OBSTACLES", self._config.MODELS_DYNAMIC_OBSTACLES) \
            or self._PROPS.model_loader.models
        model_count = len(models_list)
        average_weight = 1.0 / model_count if model_count > 0 else 0
        ...
        # 一个dynamic obstacle有2个waypoints
        waypoints_per_ped = 2
