    ...
    def laserCB(self, scan):
        ...
        if self.laser_transform.header.frame_id != "":
            ...
            for pose_type in self.agents:
                rh_vec = [
                    pose_type[0].position.x - laser_pose.x,
                    pose_type[0].position.y - laser_pose.y,
                ]
                sign = base_laser_dir[0] * -rh_vec[1] + base_laser_dir[1] * rh_vec[0]
                sign = sign / abs(sign)
                t_angle = scan.angle_max - scan.angle_min
                mid_angle = t_angle / 2 - sign * np.arccos(
                    (base_laser_dir[0] * rh_vec[0] + base_laser_dir[1] * rh_vec[1])
                    / (np.linalg.norm(rh_vec))
                )
