...
class RESNET_COVEXMAP_EXTRACTOR_1(RosnavBaseExtractor):
    """
    Feature extractor class that implements a mid-fusion ResNet-based architecture.
    ...
    def __init__(
        ...
        self._num_pedestrian_feature_maps = 1
        self._get_input_sizes()

        super(RESNET_COVEXMAP_EXTRACTOR_1, self).__init__(
            observation_space=observation_space,
            observation_space_manager=observation_space_manager,
            features_dim=features_dim,
            stacked_obs=stacked_obs,
    ...
    def _get_input_sizes(self):
        ...
        self._scan_map_size = self._observation_space_manager[SPACE_INDEX.STACKED_LASER_MAP].shape[-1]
        self._ped_map_size = self._observation_space_manager[SPACE_INDEX.CONVEX].shape[0]
        self._goal_size = self._observation_space_manager[SPACE_INDEX.GOAL].shape[-1]
    ...
    def _forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        if not self._stacked_obs:
            ...
            last_action = observations[:, -self._last_action_size :]
            # self._save_ped_pos_as_image(ped_pos)
            # # 打印 goal 和 last_action 张量
            # print("Goal:", goal)
    ...
    def _save_ped_pos_as_image(self, ped_pos_tensor, file_path="/home/dmz/Pictures/convex_map/ped_pos.png"):
        """
        将行人位置张量保存为图片，注意处理张量的维度。

        参数:
            ped_pos_tensor (torch.Tensor): 行人位置张量，可能需要从一维重塑为二维（80x80）。
            file_path (str): 图片保存的文件路径。
        """
        # 确保张量在 CPU 上，并从计算图中脱离
        ped_pos_numpy = ped_pos_tensor.cpu().detach().numpy()
        ped_pos_numpy = ped_pos_numpy.squeeze(0).reshape(80, 80)

        print(f"行人位置张量维度: {ped_pos_numpy.shape}")

        # 调整像素值范围从 [0, 30] 到 [0, 255]
        ped_pos_numpy = np.clip(ped_pos_numpy, 0, 30)  # 首先确保所有值都在 [0, 30] 范围内
        ped_pos_numpy = (ped_pos_numpy / 30.0) * 255.0
        # 使用 matplotlib 保存张量为图片
        plt.imsave(file_path, ped_pos_numpy, cmap='gray')
        time.sleep(0.5)

        # 可选，打印确认保存
        print(f"行人位置图片已保存至 {file_path}")